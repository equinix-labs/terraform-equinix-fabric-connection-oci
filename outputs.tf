...
output "oci_drg_name" {
  description = "DRG Name. It will return null if 'oci_create_drg' is false."
  value       = var.oci_create_drg ? oci_core_drg.this[0].display_name : null
}

output "oci_drg_id" {
  description = "DRG ID."
  value       = local.oci_gateway_id
}

output "oci_drg_autogenerated_vcn_route_table_id" {
  description = "DRG default route table for VCN."
  value = [
    for table in data.oci_core_drg_route_tables.this.drg_route_tables : table.id
    if length(regexall("VCN", table["display_name"])) > 0
  ][0]
}

output "oci_drg_autogenerated_generic_route_table_id" {
  description = <<EOF
  DRG default route table for all but VCN attachments: RPC, IPSec, VC. Note that for
  VCN you must reference 'oci_drg_autogenerated_vcn_route_table_id' instead.
  EOF
  value = [
    for table in data.oci_core_drg_route_tables.this.drg_route_tables : table.id
    if length(regexall("VCN", table["display_name"])) == 0
